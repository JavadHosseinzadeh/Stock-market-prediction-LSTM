...
net = trainNetwork(dataTrain(1:end-5,:).',dataTrain(6:end,1).',layers,options);
%%
...
%%
Predict=dd'.*sig(1,1)+mu(1,1);
figure,
plot(stocks.Date(1:size(Predict,1)),Data(:,1))
hold on
plot(stocks.Date(1:size(Predict,1)),Predict)
legend('Close','Predict')
xlabel('Date (day)')
ylabel('price ($)')
title('Predicted Symbol Gold')

figure,
plot(stocks.Date(end-92:end),Data(end-92:end,1))
hold on
plot(stocks.Date(end-92:end),Predict(end-92:end))
legend('Close','Predict')
xlabel('Date (day)')
ylabel('price ($)')
title('Predicted Symbol Gold')
...
%[middleBolling,upperBolling,lowerBolling]
subplot(211)
plot(Time(end-100:end),FastPercentD(end-100:end),'LineWidth' , 2)
hold on
plot(Time(end-100:end),FastPercentK(end-100:end),'LineWidth' , 2)
xlabel('Date (day)')
ylabel('Fast Stochastic Oscillator')
legend('%Fast Percent D','%Fast Percent K')
title('Symbol : Gold')
subplot(212)
plot(Time(end-100:end),SlowPercentD(end-100:end),'LineWidth' , 2)
hold on
plot(Time(end-100:end),SlowPercentK(end-100:end),'LineWidth' , 2)
xlabel('Date (day)')
ylabel('Slow Stochastic Oscillator')
