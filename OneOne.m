...
net = trainNetwork(dataTrain(1:end-1,:).',dataTrain(2:end,1).',layers,options);
%%
...
%%
dsize=size(dd,2);
for i=1:size(Data,1)-dsize-30
    [net,dd(1,dsize+i)] = predictAndUpdateState(net,dd(1,dsize+i-1),'ExecutionEnvironment','cpu');
end
Predict=dd'.*sig(1,1)+mu(1,1);
figure,
plot(2:size(Data,1)+1,Data(:,1))
